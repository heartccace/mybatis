 ...
 // 初始化的时候为parsed赋值为false，一
   ...
   private void parseConfiguration(XNode root) {
    try {
      ...
      // 将settings里面的配置赋值给Configuration的成员变量
...
private void configurationElement(XNode context) {
    ...
    // 获取当前命名空间
     ...
     // CacheRefResolver对象调用MapperBuilderAssistant对象获取当前关联缓存 如果出现异常CacheRefResolver会	// 被放入到Configuration的incompleteCacheRefs成员变量里面。MapperBuilderAssistant里的currentCache为关	// 联cache
      ...
      // 遍历所有的sql标签，并放入当前对象XmlMapperBuilder的sqlFragments
      sqlElement(context.evalNodes("/mapper/sql"));
      // 遍历所有的select|insert|update|delete标签，并解析
 ...
 // 遍历所有的parameterMap标签
    ...
    // 获取标签id
      ...
      for (XNode parameterNode : parameterNodes) {
     	 // 获取子节点property属性值 即参数中的属性名称
        String property = parameterNode.getStringAttribute("property");
         // 获取子节点javaType属性值 对应的java类型
        String javaType = parameterNode.getStringAttribute("javaType");
         // 获取子节点jdbcType属性值 对应的jdbc类型
      ...
      // 被加入Configuration的parameterMaps的成员变量里面
      builderAssistant.addParameterMap(id, parameterClass, parameterMappings);
    }
  }
```

TypeHandlerRegistry用于存储当前的所有类型转换器
...
private ResultMap resultMapElement(XNode resultMapNode, List<ResultMapping> additionalResultMappings, Class<?> enclosingType) throws Exception {
    ...
    try {
    // 将其加入到Configuration的resultMaps中
...
public void parseStatementNode() {
	...
	// 为每一条select/update/insert/delete语句创建一个MappedStatement 并放入mappedStatements中
...
自此位置关于mybatis的主配置文件和mapper配置文件加载解析完毕。

通过SqlSessionFactoryBuilder的build方法根据Configuration默认创建DefaultSqlSessionFactory(Configuration)